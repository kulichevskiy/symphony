...
from .eventdesk import eventdesk_campaign
...
def _spec_prompt() -> str:
    tickets = "\n\n".join(
        f"# {ticket.title}\n{ticket.description}" for ticket in eventdesk_campaign().tickets
    )
    return f"""You are the independent final SPEC reviewer for EventDesk.
...
def _standards_prompt() -> str:
    return f"""You are the independent final STANDARDS reviewer for EventDesk.
