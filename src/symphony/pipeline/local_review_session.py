...
async def run_local_review_session(
    *,
    ...
    async def _run_reviewer_pass(
        *,
        ...
        spec = RunnerSpec(
            ...
            workspace_path=workspace_path,
            command=command,
            # ONLY this pass's own agent credential env — reviewer/verifier
            # passes are deliberately isolated from the binding's secret env
            # (that stays fix-turn-only), and one agent's credentials never
            # reach another agent's subprocess (Config v2 3/9 + 6/9).
            env=dict((agent_envs or {}).get(agent, {})),
    ...
    async def _hybrid_reviewer(iteration: int, head_sha: str) -> ReviewerOutput:
        ...
        return ReviewerOutput(
            ...
            healthy_agents=tuple({*spec_out.healthy_agents, *bug_out.healthy_agents}),
            cost_usd=total_cost_usd,
            input_tokens=total_input_tokens,
            output_tokens=total_output_tokens,
            cache_write_tokens=total_cache_write_tokens,
    ...
    async def _reviewer(iteration: int) -> ReviewerOutput:
        ...
        if review_mode == "hybrid":
            return await _hybrid_reviewer(iteration, head_sha)
    ...
    async def _fixer(iteration: int, verdict: LocalVerdict) -> FixerOutput:
        ...
        spec = RunnerSpec(
            ...
            env={
                **(agent_envs or {}).get(fixer_role.agent, {}),
                **(binding_env or {}),
            },
        )
