...
@pytest.mark.asyncio
async def test_hybrid_strategy_skipped_local_review_starts_remote_review(
    tmp_path: Path,
    monkeypatch,
) -> None:  # type: ignore[no-untyped-def]
    import symphony.orchestrator.poll as poll_mod

    async def skipped_session(**_: object) -> LoopResult:
        return LoopResult(
    ...
    monkeypatch.setattr(poll_mod, "run_local_review_session", skipped_session)
    ...
    try:
        ...
        statuses = {run.stage: run.status for run in history}
        assert statuses["implement"] == "completed"
        assert statuses["local_review"] == "interrupted"
        assert statuses["review"] == "running"
        wait = await db.operator_waits.get(conn, "iss-1")
        assert wait is None
    finally:
        await conn.close()
