...
@pytest.mark.asyncio
async def test_tick_prunes_queue_rows_for_disabled_binding(tmp_path: Path) -> None:
    """A binding disabled in the DB is skipped by the scan loop, so its
    startup prune must drop its scope too — otherwise the stale rows from
    before it was disabled linger in the UI forever (SYM-188 review)."""
    conn = await db.connect(tmp_path / "s.sqlite")
    try:
        binding = RepoBinding(
            linear_team_key="ENG",
        ...
        assert await _rows(conn) == []
        orch._scan_binding.assert_not_awaited()  # type: ignore[attr-defined]  # noqa: SLF001
    finally:
        await conn.close()
