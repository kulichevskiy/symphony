...
async def test_linear_sandbox_creates_labeled_campaign_and_real_dependency_chain() -> None:
    responses = [
        ...
        httpx.Response(
            200,
            json={
                "data": {"issueLabelCreate": {"success": True, "issueLabel": {"id": "label-id"}}}
            },
        ),
    ]
    responses.extend(
    ...
    async with LinearSandbox("Bearer token") as sandbox:
    ...
    assert calls[1]["variables"]["input"]["name"] == "EXP-1-A1"
    assert all(call["variables"]["input"]["labelIds"] == ["label-id"] for call in calls[2:8])
