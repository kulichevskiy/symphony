...
def test_once_drains_scheduled_dispatch_before_exit(tmp_path: Path, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    async def _check() -> None:
        ...
        try:
            ...
            await conn.close()
        # implement (completed) + review monitor (running) after Implement→Review.
...
async def test_configured_tracker_registry_registers_disabled_binding_with_live_work(
    tmp_path: Path, monkeypatch
) -> None:  # type: ignore[no-untyped-def]
    """The last binding for a tracker context can be disabled while that
    context still has an active run — passing `conn` lets a tracker get
    registered anyway so reconcile can act on it (SYM-188 review)."""
    ...
    storage_id = await db.issues.upsert(
        conn, id="I1", identifier="SYM-1", title="t", team_key="SYM", provider="jira", site=site
    )
    await db.runs.create(
        conn,
        id="R1",
    ...
    # Mirrors `effective_config._resolve_bindings`, which resolves tracker
    # context defaults on every assembled binding before the daemon ever
    # builds its tracker registry.
    disabled_jira.apply_tracker_secret_defaults()
    cfg = Config(
        workspace_root=tmp_path / "ws",
        log_root=tmp_path / "logs",
        db_path=db_path,
    ...
    async with _configured_tracker_registry(cfg, conn) as (registry, _):
        assert registry.resolve(TrackerContext(provider="jira", site=site, project_key="SYM"))
    await conn.close()


async def test_configured_tracker_registry_disabled_binding_live_work_bad_creds_skips(
    tmp_path: Path, monkeypatch
) -> None:  # type: ignore[no-untyped-def]
    """Live work pins a disabled binding's context, but missing credentials
    still must not crash boot — the context is just skipped, like a disabled
    binding with no live work (SYM-188 review)."""
    monkeypatch.delenv("JIRA_BASE_URL", raising=False)
    monkeypatch.delenv("JIRA_EMAIL", raising=False)
    monkeypatch.delenv("JIRA_API_TOKEN", raising=False)
    from symphony.cli import _configured_tracker_registry
    ...
    conn = await db.connect(db_path)
    storage_id = await db.issues.upsert(
        conn, id="I1", identifier="SYM-1", title="t", team_key="SYM", provider="jira"
    )
    await db.runs.create(
        conn,
        id="R1",
    ...
    async with _configured_tracker_registry(cfg, conn) as (registry, external_linear):
        assert external_linear is None
        with pytest.raises(KeyError):
            registry.resolve(TrackerContext(provider="jira", site="default", project_key="SYM"))
    await conn.close()
